...
export const Route = createFileRoute("/automate")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "AI agents, workflows and pipelines that run your business while you sleep." },
      { property: "og:title", content: "Automation Services — Vertex Media House" },
      { property: "og:description", content: "AI agents, workflows and pipelines that run your business while you sleep." },
    ],
  }),
...
function NodeGraph({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 120 80" fill="none">
      ...
      <path d="M20 40 L54 18 M20 40 L54 62 M66 18 L100 40 M66 62 L100 40" stroke="currentColor" strokeWidth="2" strokeDasharray="3 3" />
    </svg>
  );
}
function PlusGrid({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 100 100" fill="none">
      {[20, 50, 80].flatMap((y) => [20, 50, 80].map((x) => (
...
function CircuitDoodle({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 160 40" fill="none">
      <path d="M4 20 L40 20 L48 12 L72 12 L80 20 L116 20 L124 28 L156 28" stroke="currentColor" strokeWidth="2" fill="none" />
      <circle cx="4" cy="20" r="3" fill="currentColor" />
      <circle cx="156" cy="28" r="3" fill="currentColor" />
      <rect x="40" y="8" width="40" height="8" rx="1" stroke="currentColor" strokeWidth="1.5" fill="none" />
    </svg>
  );
}
    ...
    <section className="relative w-full overflow-hidden px-4 sm:px-6 md:px-8 pt-10 md:pt-20 pb-14">
      ...
      <div className="relative mx-auto max-w-6xl">
        <div className="text-center">
          ...
          >
            <Button asChild className="rounded-full px-6 py-3 h-auto bg-neutral-900 text-white hover:bg-neutral-800 dark:bg-emerald-500 dark:text-neutral-900 dark:hover:bg-emerald-400">
            ...
            {[
              ...
              <div key={step.label} className="flex items-center gap-2">
                <div className="flex-1 flex flex-col items-center gap-2 rounded-xl border border-emerald-500/20 bg-emerald-500/5 px-3 py-4">
                  ...
                  <span className="font-mono text-xs text-neutral-700 dark:text-neutral-300">{step.label}</span>
                </div>
                {i < arr.length - 1 && (
...
const autoServices = [
  ...
  { icon: Workflow, title: "Workflow Automation", desc: "n8n, Zapier, Make — wired into your real stack." },
  { icon: Mail, title: "Email & Outreach", desc: "Triaged inboxes, personalized sends, follow-ups on auto." },
  { icon: Database, title: "CRM & Data Ops", desc: "Lead enrichment, scoring and routing in seconds." },
  { icon: Network, title: "Multi-Agent Systems", desc: "Agents that talk to each other to finish full pipelines." },
  { icon: Terminal, title: "Content Pipelines", desc: "From idea to published asset, no human in the middle." },
];
function Services() {
  return (
    <section id="services" className="relative w-full px-4 sm:px-6 md:px-8 py-16 md:py-24 bg-white dark:bg-black/40">
      ...
      <div className="mx-auto max-w-7xl">
        <div className="max-w-3xl">
          <p className="font-mono text-xs uppercase tracking-widest text-emerald-500">// services</p>
          <h2 className="mt-3 text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-neutral-950 dark:text-white">
            Modular. Composable.<br/><span className="text-emerald-500">Always-on.</span>
          </h2>
        </div>
          ...
          {autoServices.map((s, i) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono text-xs text-emerald-500/70">SYS.{String(i+1).padStart(3,"0")}</span>
              </div>
...
const builds = [
  { title: "Inbox Triage Agent", metric: "−87% reply time", desc: "AI agent that reads, classifies and drafts replies for a 12k-email/mo inbox.", stack: ["GPT-4", "n8n", "Gmail", "Notion"] },
  { title: "Content Pipeline", metric: "40 posts / week", desc: "Idea → script → caption → schedule, fully automated with human approval gate.", stack: ["Claude", "Make", "Airtable", "Buffer"] },
  { title: "Lead Scoring System", metric: "3.2× conversion", desc: "Enriches inbound leads and routes hot accounts to sales in under 60 seconds.", stack: ["Clay", "HubSpot", "Slack"] },
  { title: "Voice Booking Agent", metric: "24/7 coverage", desc: "Inbound voice agent that books, reschedules and confirms calendar slots.", stack: ["Vapi", "Cal.com", "OpenAI"] },
];
function Portfolio() {
  return (
    <section id="portfolio" className="relative w-full px-4 sm:px-6 md:px-8 py-16 md:py-24">
      ...
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12">
          <div>
            <p className="font-mono text-xs uppercase tracking-widest text-emerald-500">// case studies</p>
            <h2 className="mt-3 text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-neutral-950 dark:text-white">
              Systems we've shipped.
            </h2>
          </div>
          <p className="max-w-md text-neutral-600 dark:text-neutral-400">Real builds, real metrics. Every system is monitored and improved post-launch.</p>
        </div>
          ...
          {builds.map((b, i) => (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="font-mono text-xs text-emerald-500">BUILD/{String(i+1).padStart(2,"0")}</p>
                  <h3 className="mt-2 text-2xl font-bold text-neutral-950 dark:text-white">{b.title}</h3>
                </div>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {b.stack.map((t) => (
                  <span key={t} className="font-mono text-xs px-2 py-1 rounded-md border border-neutral-300/60 dark:border-white/10 bg-white/40 dark:bg-white/[0.03] text-neutral-700 dark:text-neutral-300">{t}</span>
                ))}
              </div>
            </SpotlightCard>
          ))}
...
/* ---------- Testimonials ---------- */
const autoQuotes = [
  { quote: "Felt like we hired five people overnight. The system just runs.", name: "Elena Rodriguez", role: "COO, NorthStack" },
  { quote: "Our sales team finally talks to leads while they're still warm.", name: "Jamal Brooks", role: "Head of Growth, Coil" },
  { quote: "Saved roughly 30 hours a week across ops. Zero new hires.", name: "Aiko Tanaka", role: "Founder, Tsuki Labs" },
];
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {autoQuotes.map((q, i) => (
            ...
            >
              <div aria-hidden className="absolute -top-6 -right-6 text-emerald-500/10 text-[140px] font-serif leading-none">"</div>
              <p className="relative text-base text-neutral-800 dark:text-neutral-200 leading-relaxed">"{q.quote}"</p>
